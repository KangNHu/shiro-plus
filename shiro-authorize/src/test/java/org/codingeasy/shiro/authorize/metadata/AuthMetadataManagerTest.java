...
public class AuthMetadataManagerTest {
	...
	public void permissionMetadataEvent() throws InterruptedException {
		...
		defaultEventBus.register(this.authMetadataManager);


		defaultEventBus.publish(new AuthMetadataEvent<PermissionMetadata>( EventType.UPDATE , new PermissionMetadata("/user" , RequestMethod.POST , Arrays.asList("user:add") , Logical.AND , PermiModel.ROLE)));
		defaultEventBus.publish(new AuthMetadataEvent<PermissionMetadata>( EventType.UPDATE , new PermissionMetadata("/user" , RequestMethod.POST , Arrays.asList("user:add:event") , Logical.AND , PermiModel.PRINCIPAL)));
		defaultEventBus.publish(new AuthMetadataEvent<GlobalMetadata>(EventType.UPDATE , new GlobalMetadata(null , Arrays.asList("/swagger") , false , true) ));
